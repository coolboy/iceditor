...
void CodeGenerator::AddDecls(List<Decl *> *decls) {
    if (CGDEBUG) printf("Adding declarations to the list of existingDecls:\n");
    for (int i = 0; i < decls->NumElements(); i++) {
        ...
        if (CGDEBUG) printf("\tAdded %s\n", cur->decl->GetName());
    }
    if (CGDEBUG) printf("Done adding declarations\n");
}

void CodeGenerator::RemoveDecls(List<Decl *> *decls) {
    if (CGDEBUG) printf("Removing declarations from the list of existingDecls:\n");
    for (int i = 0; i < decls->NumElements(); i++) {
...
Location *CodeGenerator::GenTempVar()
{
  ...
  Location *result = new Location(fpRelative, CodeGenerator::OffsetToFirstLocal - CodeGenerator::VarSize * curNumLocals++, temp);
  Assert(result != NULL);
  return result;
}
