...
void
	STInit()
{
	int nStrInd/* string table index */, nSymInd; 

	nStrInd = put_in_table("system",IDtype,6); /* return string index of string "system" */
...
*/
int
	InsertEntry(int id) //in string table index | out symbol table index
{
...
*/
int
	LookUp(int id) //in string table id | out symbol table id
{
	int i;
...
void STPrint()
{
	...
	for (i = 1; i <= st_top; i++)
	{
		...
		for (attr_num = NAME_ATTR; attr_num <= ARGNUM_ATTR; attr_num++)
			if ((attr_ptr = IsAttr(i, attr_num)) != 0)
			{
...
int ReverseLookUp( int st_index)
{
